...
import { clearChequeAction, createPaymentAction, deleteInvoiceAction, deletePaymentAction, markInvoicePaidAction, updatePaymentAction } from "@/lib/actions";
...
export default async function InvoiceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <>
      <PageHeader
        title={`Invoice · ${invoice.shop.name}`}
        description={`${displayDate(invoice.invoiceDate)} · remaining ${money(remaining)}`}
        action={
          <div className="flex gap-2">
            <LinkButton href={`/shops/${invoice.shopId}`} variant="secondary">Shop account</LinkButton>
            {invoice.allocations.length === 0 ? <LinkButton href={`/invoices/${invoice.id}/edit`} variant="secondary">Edit invoice</LinkButton> : null}
      ...
      <div className="grid gap-5 xl:grid-cols-[1fr_380px]">
        <Panel>
          ...
          {invoice.trip ? <p className="mt-3 text-sm text-muted">Linked trip: {displayDate(invoice.trip.tripDate)} · {invoice.trip.vehicle.nameOrNumber}</p> : null}
        </Panel>
        <Panel>
